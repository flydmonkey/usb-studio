...
If you enable LAN playback, the app starts a local **unencrypted HTTP** server on your Wi-Fi/LAN so other devices on the same network can watch silent live preview or saved recordings, and download those recordings. Anyone on that network who knows the URL can open it. This traffic is not sent to the developer and is not HTTPS.
...
若打开局域网播放，应用会在当前局域网启动**未加密的 HTTP** 服务，同一网络上的其他设备可以用浏览器看无声现场画面或已录成片，并下载这些成片。知道地址的人都可以打开。流量不会发到开发者，也不使用 HTTPS。
